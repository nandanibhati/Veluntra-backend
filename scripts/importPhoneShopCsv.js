...
// Approximate hex swatches for marketing colour names (e.g. "Icy Blue", "Titanium Grey") — the
// UI renders a real swatch from ProductOption.extra, falling back to a flat grey when it's null,
// which makes every colour option look identical. Checked most-specific-phrase-first so e.g.
// "rose gold" doesn't fall through to the plain "gold" match.
  ...
  for (const [handle, rows] of byHandle) {
    try {
      ...
      if (variantRows.length > 1) {
        ...
        options = distinctColours.map((label) => ({ kind: "color", label, extra: colorNameToHex(label), inStock: true }));
        variants = [];
